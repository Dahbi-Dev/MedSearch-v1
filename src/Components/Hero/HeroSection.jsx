...
const HeroSection = ({ onSearchResults, onSearchStart, onClearSearch, user }) => {
  ...
  const fetchCities = async () => {
    try {
      const response = await fetch('https://medsearch-server-production.up.railway.app/api/doctors');
      if (response.ok) {
  ...
  const handleSearch = async (search = searchTerm, city = selectedCity) => {
    try {
      ...
      const response = await fetch(`https://medsearch-server-production.up.railway.app/api/doctors?${params.toString()}`);
      if (response.ok) {
        const doctors = await response.json();
